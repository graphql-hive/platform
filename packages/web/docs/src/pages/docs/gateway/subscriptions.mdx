...
<Callout type="info">
  This is also what Apollo Router do when [propagating auth on
...
For the sake of brevity, we'll subscribe using `curl`:
...
Apollo Client is not compatible with SSE out of the box, but it's straightforward to add an Apollo
Link to enable this feature by using [`graphql-sse`](https://the-guild.dev/graphql/sse) library.
...
Relay doesn't support SSE out of the box, but you can easily enable this feature by setting up a
[Network Layer](https://relay.dev/docs/guides/network-layer/) using
...
You can subscribe to the gateway through WebSockets. For the sake of brevity, we'll subscribe using
`wscat`:
...
npx wscat \
  ...
  -s graphql-ws \
  -x '{"type": "connection_init"}' --wait 600
```
...
Apollo Client supports WebSocket subscriptions over WebSocket by using the
...
You will need to use a `split` link to either use classic HTTP for queries and mutation, or
...
## Subgraph subscriptions using WebSockets
...
## Advanced subscriptions transport configuration
