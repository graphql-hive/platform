...
description:
  Hive is an open source drop-in replacement for Apollo GraphOS with no
  vendor lock-in. Managed and 100% on-premise options.
---

# Open Source Apollo GraphOS Alternative

Hive stands as a **compelling alternative to Apollo GraphOS**. With a
various of features and benefits, Hive has proven itself as an exceptional choice for both
...
For those fimiliar with using Rover CLI, transitioning to Hive is remarkably
straightforward. The [Hive CLI](/docs/api-reference/cli) has a **similar interface, minimizing the
learning curve for existing users**. This aspect simplifies the migration process from Apollo GraphOS
to Hive, allowing developers to quickly adapt and continue their work without interruption.
...
Whether you're working on a side project or managing a larger beast, Hive caters to your
